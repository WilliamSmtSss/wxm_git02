...
public class ResultUtil {
    ...
    public static void sendJsonMessage(HttpServletResponse response, String code, String msg) throws Exception {
        response.setContentType("application/json; charset=utf-8");
        PrintWriter writer = response.getWriter();
        CommonResult commonResult = returnError(code,msg);
        writer.print(JSONObject.toJSONString(commonResult, SerializerFeature.WriteMapNullValue,
                SerializerFeature.WriteDateUseDateFormat));
        writer.close();
        response.flushBuffer();
    }
