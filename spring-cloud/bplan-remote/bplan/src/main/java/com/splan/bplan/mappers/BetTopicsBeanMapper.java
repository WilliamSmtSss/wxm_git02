...
public interface BetTopicsBeanMapper extends SuperMapper<BetTopicsBean> {
    ...
    @Results({
            ...
            @Result(column = "id",property = "betOptions",many=@Many(select = "com.splan.bplan.mappers.BetOptionBeanMapper.selectByBetDataId"))
    })
    List<BetTopicsBean> selectSeriByTopicableId(Integer topicableId);


    @Select("SELECT a.*,number,c.game_id FROM bet_topics as a LEFT JOIN game_campaign as b on a.topicable_id=b.id LEFT JOIN game_data as c  on b.data_id=c.id where a.topicable_type='Campaign' and " +
            "b.data_id=#{topicableId} and a.status!='canceled' and a.rolling_ball=1   UNION ALL select a.*,0 as number,c.game_id from bet_topics as a left join game_data as c on a.data_id=c.id where a.topicable_id=#{topicableId} and a.topicable_type='Series' and a.status!='canceled' and a.rolling_ball=1 ")
    @Results({
            @Result(id=true,property="id",column="id",javaType=Integer.class),
            @Result(column = "id",property = "betOptions",many=@Many(select = "com.splan.bplan.mappers.BetOptionBeanMapper.selectByBetDataId"))
    })
    ...
    @Select("select *,id from bet_topics where data_id=#{dataId} and topicable_type='Series' and category=1 and support=0 union all \n" +
            "select *,id from bet_topics where data_id=#{dataId} and topicable_type='Series' and category=1 and support=11 order by rolling_ball desc limit 0,1")
