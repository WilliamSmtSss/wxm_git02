...
import com.splan.bplan.dto.BetBatchNotifyDto;
import com.splan.bplan.dto.BetNotifyDto;
import com.splan.bplan.dto.RollbackBetNotifyDto;
import com.splan.bplan.dto.SimpleBetNotifyDto;
import com.splan.base.enums.*;
import com.splan.bplan.http.CommonResult;
import com.splan.bplan.mappers.*;
import com.splan.bplan.result.BatchBetResult;
import com.splan.bplan.result.BetOrderResult;
import com.splan.bplan.result.OperateOrderResult;
import com.splan.bplan.result.SingleBetResult;
...
public class BetOrderServiceImpl extends ServiceImpl<BetOrderBeanMapper, BetOrderBean> implements IBetOrderService {
    ...
    private void postOrder(List<BetOrderBean> list){
        ...
        try {
            ...
            map.forEach((key,value)->{
                try{
                    ProxyConfig proxyConfig = proxyConfigProvideService.selectProxyConfigByClientId(key);
    ...
    private String rollbackPost(List<BetOrderBean> value){
        ...
        for (int i = 0; i < value.size(); i++) {
            ...
            if (StringUtils.isBlank(userBean.getApiId())){
                return null;
            }
            ProxyConfig proxyConfig = proxyConfigProvideService.selectProxyConfigByClientId(userBean.getApiId());
            if (proxyConfig!=null) {
    ...
    private void winSettle(BetOrderDetailBean betOrderDetailBean){
        ...
        betOrderDetailBean.setCheckTime(new Date());
        ...
        if (betOrderDetailBean.getOrderType()==Boolean.TRUE){
            if (betOrderDetailBean.getBetOrderBean().getStatus().equals(OrderStatus.UNSETTLED)){
                ...
                if (count==0){//最后一个结算,更新主表数据
                    ...
                    betOrderDetailBean.getBetOrderBean().setCheckTime(new Date());
                    betOrderDetailBean.getBetOrderBean().setUpdateTime(new Date());
                    ...
                    noticeService.sendNotice("win",betOrderDetailBean.getBetOrderBean().getTenantOrderNo(),betOrderDetailBean.getBetOrderBean().getEstimatedReward(),"",betOrderDetailBean.getBetOrderBean().getCreateTime(),null,betOrderDetailBean.getBetOrderBean().getId(),betOrderDetailBean.getBetOrderBean().getTenantUserNo());
                }
            }
        }else {
            betOrderDetailBean.getBetOrderBean().setWinLose(WinLoseStatus.WIN);
        ...
        if (betOrderDetailBean.getOrderType()==Boolean.TRUE){
            if (betOrderDetailBean.getBetOrderBean().getStatus().equals(OrderStatus.UNSETTLED)){
                ...
                if (count==0){//全部取消
                    ...
                    noticeService.sendNotice("refund",betOrderDetailBean.getBetOrderBean().getTenantOrderNo(),betOrderDetailBean.getBetOrderBean().getEstimatedReward(),title,betOrderDetailBean.getBetOrderBean().getUpdateTime(),null,betOrderDetailBean.getBetOrderBean().getId(),betOrderDetailBean.getBetOrderBean().getTenantUserNo());
                }else {
                    //重新计算赔率
                    //取消
        ...
        for (int i = 0; i < orders.length; i++) {
            ...
            if (betOrderBean.getStatus().equals(OrderStatus.UNSETTLED)){
                for (int j = 0; j < betOrderBean.getBetOrderDetails().size(); j++) {
                    ...
                    betOrderBean.getBetOrderDetails().get(j).setCheckTime(new Date());
                ...
                betOrderBean.setCheckTime(new Date());
                betOrderBean.setUpdateTime(new Date());
                betOrderBeanMapper.updateById(betOrderBean);
                process(betOrderBean,AlgorithmType.ADD,false);
                noticeService.sendNotice("refund",betOrderBean.getTenantOrderNo(),betOrderBean.getEstimatedReward(),"盘口取消",betOrderBean.getUpdateTime(),null,betOrderBean.getId(),betOrderBean.getTenantUserNo());
            }
