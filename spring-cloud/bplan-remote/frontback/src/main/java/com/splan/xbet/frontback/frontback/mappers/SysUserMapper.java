...
public interface SysUserMapper extends SuperMapper<SysUser> {
     ...
     * 修改密码
     */
    @Update("update back_sys_user set password =#{password} where username=#{username}")
     ...
     * 联合角色查询
     */
    @Select("select t1.*,t2.role_name from back_sys_user t1,back_sys_role t2 where t1.role_id=t2.id where t1.fromid=#{userId}")
    List<SysUser> selectJoinRole(@Param("userId") String userId);

    //xBet
    @Select("<script>select t1.*,t2.role_name,ifnull(t3.username,'') as modify_name from back_sys_user t1 left join back_sys_role t2 on t1.role_id=t2.id left join back_sys_user t3 on t1.modify_id =t3.id<where> 1=1 " +
    ...
    @Update("update back_sys_user t set t.role_id=#{roleId} where t.id=#{sysUserId}")
    int updateXbetSysUserRoleId(String sysUserId, String roleId);

    @Select("<script>select t1.*,t2.role_name,ifnull(t3.username,'') as from_name,ifnull(t4.username,'') as modify_name from back_sys_user t1 left join back_sys_role t2 on t1.role_id=t2.id left join back_sys_user t3 on t1.fromid=t3.id left join back_sys_user t4 on t1.modify_id=t4.id<where> 1=1 " +
