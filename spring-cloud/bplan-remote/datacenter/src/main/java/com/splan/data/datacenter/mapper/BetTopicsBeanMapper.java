...
//            END
    ...
    //xBet
    @Select("<script>" +
            "select * from x_bet_view t <where>" +
            " <if test='gameName != null '> AND t.gameId=#{gameId} </if> "+
            " <if test='startDate != null '> AND unix_timestamp(t.update_time) <![CDATA[>=]]> unix_timestamp(#{startDate}) </if> "+
            " <if test='endDate != null '> AND unix_timestamp(t.update_time) <![CDATA[<=]]> unix_timestamp(#{endDate}) </if> "+
            " <if test='dataId != null '> AND t.data_id=#{dataId} </if> "+
            " <if test='betId != null '> AND t.id=#{betId} </if> "+
            " <if test='betStatus != null '> AND t.status=#{betStatus} </if> "+
            "</where>"+
            "</script>")
    List<BetTopicsBean> getXbetBetData(Page page, String gameId, String startDate, String endDate, String dataId, String betId, String betStatus);
}